...
const useInitLocalApi = () => {
  window.localApi.onSave((_, value: Local.LocalSaveParams<{ name: string; value: any }>) => {
  ...
  window.localApi.onUpdate((_, value: Local.LocalSaveParams<{ name: string; value: any }>) => {
    console.log(value)
    const request = PlutoIndexDb.db
      .transaction([value.name], value.options?.type || 'readwrite')
      .objectStore(value.name)
